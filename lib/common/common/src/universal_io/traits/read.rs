...
pub trait UniversalRead: Sized + Debug + Send + Sync {
    ...
    /// The single read-pipeline implementation for this backend.
    ///
    /// The *owned* (file-owning) pipeline is derived generically as
    /// [`OwnedPipeline<Self::ReadPipeline<'static, U>, Self, U>`](super::OwnedPipeline);
    /// backends do not implement it separately.
